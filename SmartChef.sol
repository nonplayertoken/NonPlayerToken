...
// added functionnalities by the npct's devs
//
...
contract SmartChef is Ownable {
    ...
    // The block number when NPCT mining ends.
    ...
    function pendingReward(address _user) external view returns (uint256) {
        ...
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            ...
            uint256 npctReward = multiplier.mul(rewardPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
    ...
    // Stake NPCT tokens to TheBushV1
    ...
    // Withdraw NPCT tokens from STAKING.
